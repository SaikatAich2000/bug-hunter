"""Tests for the app's infrastructure modules rather than its CRUD endpoints:
...
stale. Either (a) use the ``client`` / ``admin_client`` fixtures (which trigger
the reimport and leave ``app.*`` pointing at the live generation), then import
``app.<mod>`` inside the test, or (b) for the module-import-time branches in
app.main (CORS '*', etc.) drive a fresh import explicitly with the env pre-set
via ``_fresh_app``.
...
def test_cov_push_register_new_then_refresh_and_cross_user_conflict(admin_client):
    # push_service register: insert a fresh token, refresh it for the same user,
    # then verify a different user is refused (PushTokenConflict) rather than
    # silently re-homed, as a hijack defense.
    import pytest
    from app import push_service
    ...
    # Same-user re-register just refreshes metadata (no duplicate row).
    ...
    # Create a second user; a cross-user claim of the same token is refused.
    db.close()
    ...
    # Still exactly one row, still bound to the original owner.
