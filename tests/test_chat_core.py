"""Tests for the chatbot's NLU parser (app.chatbot.nlu) and command executor
(app.chatbot.executor).

The cloud LLM layer is off in the test suite (SLEUTH_CLOUD_ENABLED=0, set in
conftest), so every message runs the deterministic local chain:

    parse() -> rule dispatch -> classifier -> (LLM unavailable) -> unknown

The `client` fixture re-imports every ``app.*`` module per test so the SQLite
engine picks up the per-test env override. We therefore import ``app.*`` inside
each test (module-level refs go stale) and build a real ``Session`` via the
freshly-imported ``app.database.SessionLocal``. Most branches are reached by
calling ``executor.execute(message, db, actor)`` and ``nlu.parse(message,
ctx)`` directly, with a handful of end-to-end checks through
``POST /api/chat/ask``.

Hermetic: no network, no real LLM, and no openpyxl dependency required (the
export path degrades gracefully and we assert whichever branch actually fires).
"""
...
def test_cov_action_helpers_negative_paths():
    ...
    # priority change requires both the regex and a parsed priority
...
def test_cov_exec_about_known_and_unknown(client):
    ...
    try:
        ...
        # The "priorit" stem matches both "priority" and the plural
        # "priorities" (the y→ies change means a bare "priority" key would miss
        # the plural), and both get the full answer including the P0→Critical
        # aliases, not a shorter stub.
...
# ===========================================================================
# End-to-end through the HTTP router. Confirms the executor entry point is
# reachable via the real API.
# ===========================================================================
...
def test_cov_changeme_password_still_valid():
    # 'changeme' is always accepted by the policy.
...
def test_cov_nlu_action_helpers_absent_body_branches():
    ...
    # comment verb WITH a bug target but no colon → body regex doesn't match →
    # body None. (A comment now requires a target; without a bug_id/pronoun it's
    # treated as a read, see the no-target assertion below.)
    ...
    # No bug target → not a write ("any comment on the latest release?" is a
