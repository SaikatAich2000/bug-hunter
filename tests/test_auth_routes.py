...
import generation the running app is using. To patch app.config we set the
``Settings`` class attribute (module-level instances go stale).

The reset-flow tests insert PasswordResetToken rows directly through the app's
own ``SessionLocal`` using ``generate_reset_token()`` so we hold the raw token
without reading an email, letting us deterministically build valid, expired,
already-used, and unknown-token cases. The email backend is disabled in tests,
...
def test_cov_forgot_password_known_email_creates_token(client):
    ...
    Exercises the real-account branch: generate token, persist, audit, queue
    the background email. The email task is a no-op against the disabled
    backend, so this stays hermetic.
    """
...
def test_cov_forgot_password_unknown_email_204_default(client):
    ...
    known one, so callers can't probe which emails exist."""
    res = client.post("/api/auth/forgot-password",
...
def test_cov_reset_password_valid_token_succeeds(client):
    ...
    Drives the happy path of reset_password: expiry and used checks pass, user
    lookup, password hash, session_version bump, token marked used, session
    rows wiped.
    """
    uid = _admin_user_id(client)
    client.post("/api/auth/logout")
...
def test_cov_reset_password_used_token_rejected(client):
    """An already-used token is rejected with 400 (the ``used_at is not None``
    branch)."""
    uid = _admin_user_id(client)
    client.post("/api/auth/logout")
...
def test_cov_reset_password_unknown_token_rejected(client):
    """A token that hashes to no stored row is rejected with 400 (the
    ``prt is None`` guard)."""
    res = client.post("/api/auth/reset-password",
...
def test_cov_reset_password_inactive_user_rejected(client, admin_client):
    ...
    400 (the ``not user.is_active`` branch)."""
...
def test_cov_reset_password_naive_expiry_is_handled(client):
    """If a stored token's expires_at is tz-naive (a SQLite round-trip),
    reset_password must coerce it to UTC before comparing rather than raising.
    We write a naive future timestamp directly and confirm the reset still
    succeeds."""
    uid = _admin_user_id(client)
    client.post("/api/auth/logout")
...
def test_cov_login_wrong_password_401(client):
    """Real user, wrong password returns a unified 401."""
    res = client.post("/api/auth/login", json={
...
def test_cov_login_inactive_user_blocked(client, admin_client):
    """A deactivated account cannot log in even with the right password: the
    same unified 401, with no account-state leak."""
    created = admin_client.post("/api/users", json={
...
def test_cov_login_lockout_returns_429(client):
    ...
    short-circuits with 429 before the bcrypt verify. We pre-fill the bucket
    via the lockout module so the assertion is deterministic and independent
    of the IP limiter."""
...
def test_cov_logout_token_user_deleted_skips_audit(client, admin_client):
    ...
    session row keyed by jti is deleted.
...
def test_cov_logout_legacy_cookie_without_jti(client):
    ...
    in logout: the user audit row is written but no per-session delete runs.
    We craft a valid 2-part token for the admin and present it as the session
    cookie."""
    uid = _admin_user_id(client)
    client.post("/api/auth/logout")
...
def test_cov_logout_without_cookie_is_idempotent(client):
    ...
    DB block, still returning 204. Logout is idempotent."""
...
def test_cov_client_ip_trust_enabled_empty_xff_falls_through(monkeypatch):
    ...
    _client_ip skips the XFF branch and falls back to the transport client
    host. Exercised directly with a synthetic Request so it's hermetic and
    independent of the TestClient transport."""
...
def test_cov_client_ip_trust_enabled_uses_rightmost_xff(monkeypatch):
    """With TRUST_PROXY_FORWARDED_FOR=True (single proxy hop) and a populated
    X-Forwarded-For, _client_ip returns the right-most (proxy-appended) entry,
    not the client-controlled left-most one, which is attacker-supplied and
    must not be trusted for the audit/session IP."""
    import app.config as config
    monkeypatch.setattr(config.Settings, "TRUST_PROXY_FORWARDED_FOR", True)
...
def test_cov_client_ip_trust_enabled_garbage_xff_falls_through(monkeypatch):
    ...
    ValueError, the except swallows it, and _client_ip falls back to the real
    transport client host rather than persisting the spoofed label."""
    import app.config as config
    monkeypatch.setattr(config.Settings, "TRUST_PROXY_FORWARDED_FOR", True)
...
def test_cov_client_ip_no_client_returns_empty(monkeypatch):
    ...
    The trust gate is left at its default False so we go straight to the client
    check."""
    from app.routes.auth import _client_ip
    from starlette.requests import Request
...
def test_cov_me_missing_cookie_401(client):
    """No session cookie: get_current_user raises 401 via _user_from_request
    returning None on a missing cookie."""
...
def test_cov_me_expired_token_401(client):
    ...
    (parse_session_token raises SignatureExpired, a BadSignature subclass) and
    returns 401.

    Rather than wait out a real day, we mint the cookie with a signer that
    backdates its own timestamp well into the past, using the same secret and
    salt the app's _signer() uses. The live verifier (default 86400s TTL) then
    sees it as expired, so no real time elapses."""
...
def test_cov_parse_session_token_non_utf8_payload_returns_none(client):
    ...
    parse_session_token swallows the UnicodeDecodeError and returns None. The
    signature check must pass first, so we sign genuine non-UTF-8 bytes with
    the live signer and feed the signed bytes straight in (decoding the token
    to str would itself fail and never reach the function). itsdangerous
    re-returns the original bytes from unsign(), and it's the subsequent
    raw.decode('utf-8') that raises."""
...
def test_cov_expired_session_row_deletes_and_rejects(client):
    ...
    rejected (_validate_session_row) and the stale row deleted in-line
    (_delete_expired_session).
...
def test_cov_naive_session_expiry_is_coerced(client):
    ...
    be coerced to UTC and accepted (the naive-to-aware branch), not crash the
    request. /me must succeed."""
    uid = _admin_user_id(client)
    ...
    try:
        ...
        # path actually runs on this request.
...
def test_cov_last_seen_bump_runs_after_throttle_window(client):
    ...
    writes a fresh last_seen_at. We back-date last_seen_at (tz-aware) far
    enough that the next authenticated request bumps it, and assert it moved
    forward."""
    uid = _admin_user_id(client)
...
# ---------------------------------------------------------------------------
# Direct-call tests for the session-row helpers. These exercise the tz-aware
# arms and the SQLAlchemyError handlers that the HTTP path can't reach on
# SQLite: the SQLite dialect returns DateTime(timezone=True) columns as
# tz-naive on read, so a round-tripped row always takes the naive branch, and
# a healthy in-memory DB never makes commit() raise. We build SessionRow
# objects in memory (aware datetimes survive) and, for the error paths, wrap a
# real session whose commit() is monkeypatched to raise.
# ---------------------------------------------------------------------------
...
def test_cov_validate_session_row_aware_expiry_branch(client):
    """_validate_session_row with a tz-aware, future expires_at skips the naive
    coercion and returns True. We set the in-memory attribute to an aware
    datetime so it isn't a SQLite round-trip."""
...
def test_cov_maybe_bump_last_seen_aware_recent_is_noop(client):
    """_maybe_bump_last_seen with an aware, recent last_seen_at skips the naive
    coercion and then early-returns at the throttle guard without writing."""
...
def test_cov_delete_expired_session_commit_error_is_swallowed(client, monkeypatch):
    ...
    logs and rolls back rather than propagating. We patch commit on the live
    session to raise; rollback must be called and no exception escapes."""
...
def test_cov_maybe_bump_last_seen_commit_error_is_swallowed(client, monkeypatch):
    ...
    logs and rolls back. We force last_seen_at old enough to pass the throttle
    guard, then make commit raise."""
    from sqlalchemy.exc import SQLAlchemyError
    from app.auth import _maybe_bump_last_seen
    uid, jti, db, sess = _persisted_session_row(client)
    try:
...
def test_cov_get_current_user_optional_paths(client):
    ...
    User for an authenticated one. Exercised directly with a synthetic Request
    and a real DB session so both return arms run."""
    uid = _admin_user_id(client)
...
def test_cov_require_manager_or_admin_forbids_user():
    """require_manager_or_admin raises 403 for a plain user and returns the
