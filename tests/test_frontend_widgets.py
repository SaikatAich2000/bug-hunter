"""Source guards for several frontend behaviors.

Covered here:

  1. Reports chip checkbox UI: native checkbox hidden, custom indicator span
     rendered.
  2. Comment-attachment Delete button does not submit the enclosing bug form
     (carries type="button").
  3. Comment composer files-only does not leak as a bug-level attachment:
     postComment() rejects with a toast and directs the user to the
     bug-level uploader.
  4. Reports view uses the app's theme tokens instead of white panel
     fallbacks.
  5. Application version reported consistently everywhere.

These invariants cannot run through a TestClient (no DOM), so they are
asserted by sniffing the source files.
"""
...
def test_delete_attachment_button_has_type_button():
    """A button inside <form> with no `type` attribute defaults to `submit` and
    triggers a form submit on click. type="button" ensures deleting a comment's
    attachment doesn't also save the bug and close the modal."""
...
def test_post_comment_rejects_files_without_body():
    """postComment() keeps comment uploads and bug-level uploads separate:
    files attached in the comment composer must accompany comment text, or the
    composer refuses with a toast pointing the user at the bug-level
    uploader."""
...
def test_reports_chip_indicator_span_is_rendered():
    """The chip indicator (the checkmark dot) lives in its own span. CSS hides
    the native checkbox via absolute positioning and opacity:0 and shows the
    indicator span only when checked, via :has(input:checked). If either side
    breaks, the chip falls back to a raw checkbox."""
...
def test_reports_view_uses_theme_tokens(selector_block, must_contain):
    """Each Reports block must use the app's theme variables so the panel
    matches the dark/light theme, rather than a white (`#fff`) fallback."""
    css = STYLES_CSS.read_text(encoding="utf-8")
...
# both typed characters and toolbar formatting commands. The browser's native
...
# (comments, audit log, session list, bug modal metadata), with no today-only
# shortcut that hides one half.
# ---------------------------------------------------------------------------
