"""Coverage slice: Sleuth chatbot actions / router / excel / redaction /
classifier error-and-edge branches.

This file targets the *unhappy paths* the happy-path suites
(test_sleuth_actions, test_sleuth_comprehensive, etc.) don't reach:
permission denials, not-found targets, invalid field values, the rollback
exception handler, router rate-limit eviction + transcript persistence +
graceful executor-crash handling, the Excel cache eviction / expiry paths,
the redaction fail-closed branch, and the classifier's empty-example skip.

Conventions (mirrors test_password_policy.py + conftest.py):
  ...
  - To toggle a config flag we set the ``config.Settings`` CLASS attribute
    via ``monkeypatch`` (module-level refs go stale on reimport).
...
def test_cov_create_bug_inactive_account(client):
    ...
    try:
        ...
        resp = actions.execute_plan(plan, db, bob)
...
def test_cov_assign_bug_not_found_and_perm_denied(client):
    ...
    try:
        ...
        r_missing = actions.execute_plan(plan_missing, db, alice)
        ...
        if hasattr(bug, "item_type"):
            ...
            r_perm = actions.execute_plan(plan_perm, db, bob)
...
def test_cov_unassign_branches(client):
    ...
    try:
        ...
        # 256 — bug missing.
        r_missing = actions.execute_plan(
        ...
        # 265 — no-op: drop a user that isn't assigned (carol id never on bug).
        r_noop = actions.execute_plan(
            actions.ActionPlan(kind="unassign", actor_user_id=alice.id,
                               bug_id=ids["bug_id"],
        ...
        assert r_noop.intent == "action_error"
...
def test_cov_set_field_permission_denied(client):
    ...
    try:
        ...
        db.commit()
        bob = db.get(models.User, ids["bob_id"])
        # set_status routes through _apply_set_field — bob can't edit a Requirement.
...
def test_cov_excel_evict_expired(client):
    ...
    excel._cache["dead"] = (b"x", "a.xlsx", now - 1)
    excel._cache["live"] = (b"y", "b.xlsx", now + 1000)
...
def test_cov_excel_evict_oldest_over_cap(client):
    ...
    for i in range(cap):
        excel._cache[f"tok{i}"] = (b"x", f"f{i}.xlsx", base + i)
...
def test_cov_excel_fetch_staged_empty_token_and_expired(client, monkeypatch):
    ...
    # Entry already expired relative to `frozen` (expires < now).
    excel._cache["stale_tok"] = (b"data", "f.xlsx", frozen - 1)
    assert excel.fetch_staged("stale_tok") is None      # 208 True -> 209-210
    assert "stale_tok" not in excel._cache              # popped on 209
    excel.clear_all_for_test()
