...
def test_reclaim_deletes_idle_bucket():
    ...
    assert "idle@x" not in account_lockout._buckets  # del (line 85) + len<MAX exit
    account_lockout._reset_for_tests()
...
def test_reclaim_evicts_soonest_expiring_when_all_locked(monkeypatch):
    ...
    # Two LOCKED buckets (locked_until far in the future, recent fails → not idle).
    ...
    account_lockout._reclaim_buckets(1000.0)
    # No idle buckets, all locked → drop the soonest-to-expire (a@x) as backstop.
...
def test_strip_preserves_animated_image(monkeypatch):
    ...
    # A multi-frame image must be returned untouched (a plain save() would drop
    # all but the first frame), so strip returns the original bytes verbatim.
...
def test_scheduler_start_reentry_is_noop(monkeypatch):
    ...
    scheduler.start()  # already running → logs + returns, doesn't replace _task
